...
% A
figure('name', 'a');
imshow(uint8(real(fouriecomp(A, 150))));

% B
figure('name', 'b');
imshow(uint8(real(fouriecomp(A, 65))));
...
hold on
plot(log10(abs(F)), 'b');
m_compen = zeros(501,1);
for i=1:500
    m_compen(i) = sqrt(501*2*i+i);
end
plot(m_compen, log10(abs(F)), 'r');
toc
